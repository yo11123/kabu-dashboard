...
| 指標 | 意味 | 見方 |
|------|------|------|
| **複合スコア** | ML予測+テクニカル+ファンダを統合した総合評価（0〜100） | 70以上＝有望、50以下＝注意 |
| **テクニカル** | RSI・MACD・一目均衡表・ADX・BB等9指標の加重スコア（0〜100） | チャートの勢いを数値化 |
| **ファンダ** | PER・PBR・配当・ROE等＋セクター別コモディティ連動性 | A+〜Dの6段階。企業価値の割安度 |
| **方向予測(ML)** | XGBoostが**5日後に+2%以上上昇する確率**を予測 | 60%以上＝上昇の可能性が高い。東証全銘柄4,000社のデータで学習済み |
| **タイミング(ML)** | XGBoostが**10日後に+3%以上上昇する確率**を予測 | 60%以上＝今が買い時の可能性。ファンダメンタル+ニュースセンチメントも加味 |

**方向予測とタイミングの違い:**
- **方向予測** → 短期（5日）の値動きの方向を予測。「上がるか下がるか」
- **タイミング** → 中期（10日）で十分な利益が出るかを予測。「今買うべきか」

どちらも60%を超えると信頼度が高く、両方60%以上なら強い買いシグナルです。
""")
