...
## 読み取る項目（この2つだけ）
1. **銘柄コード**: 4桁の数字（例: 7203, 3350）。銘柄名の下に表示される
2. **保有株数**: 「○○株」と表示される数字。「(0)」等の注文数は無視
3. **平均取得単価**: 「取得単価」と明記されている数字のみ

## 証券アプリごとのレイアウト解説

### SBI証券（スマホアプリ）
画面は2つのタブで切り替わる:
- **保有株数タブ**（ヘッダー: 銘柄/保有株数/評価額）
  - 左列: 銘柄名 + コード + 預り区分(NISA/特定)
  - 中央: 「○○株」= 保有株数、「(0)」= 注文数（無視すること）
  - 右列上: 評価額（円）← これは取得単価ではない！
  - 右列下: 評価額前日比（±円, ±%）← これも取得単価ではない！
- **現在値・取得単価タブ**（ヘッダー: 銘柄/現在値・取得単価/評価損益）
  - 中央上: 赤or白の数字 = **現在値**（取得単価ではない！）
  - 中央下: 白の「○○.0円」= **取得単価** ← これだけがavg_cost
  - 右列: 評価損益（±円, ±%）← これは取得単価ではない！

### 楽天証券
- 「平均取得価額」のラベルがある数字が取得単価
- 「時価評価額」「評価損益」は取得単価ではない

### マネックス証券
- 「買付単価」のラベルがある数字が取得単価

## 絶対に守るルール
- **評価額は取得単価ではない**（例: 271,350円は100株の評価額であり、1株の取得単価ではない）
- **現在値は取得単価ではない**（例: 赤字の2,713.5円は現在値であり取得単価ではない）
- **取得単価がこの画像に表示されていない場合は avg_cost を 0 にする**
- 数値のカンマは除去して数値にする
- 小数点がある場合（3,150.0円等）はそのまま数値にする
- 株数は整数
...
def main() -> None:
    ...
    st.markdown("<h1 style='font-family:Cormorant Garamond,serif; font-weight:300; letter-spacing:0.12em; font-size:1.6rem;'>ポートフォリオ分析</h1>", unsafe_allow_html=True)
    st.caption(
